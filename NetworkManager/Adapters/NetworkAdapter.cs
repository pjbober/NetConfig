...
namespace NetworkManager
{
    ...
    public static class NetworkAdapterEnumerator
    {
        ...
        public static IList<NetworkAdapter> GetAllNetworkAdapters()
        {
            ...
            foreach (ManagementObject result in search.Get())
                adapters.Add(new NetworkAdapter(result));

            return adapters;
        }

        public static NetworkAdapter GetNetworkAdapter(int id)
        {
            SelectQuery query = new SelectQuery("Win32_NetworkAdapter", "InterfaceIndex=" + id);
            ManagementObjectSearcher search = new ManagementObjectSearcher(query);

            var result = (IEnumerable<ManagementObject>)search.Get();
            ManagementObject obj = result.FirstOrDefault();

            if (obj != null)
                return new NetworkAdapter(obj);
            else
                return null;
        }
